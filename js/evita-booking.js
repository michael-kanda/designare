...
export function createBookingModal(state, ApiHandler, ModalController) {
    ...
    const BookingModal = {
                    ...
                    data.suggestions.forEach(suggestion => {
                        ...
                        button.innerHTML = `
                            <div style="flex-grow: 1;">
                                <div style="font-weight: 500;">${escapeHtml(suggestion.formattedString.split(' um ')[1] || '')}</div>
                                <div style="font-size: 0.9rem; opacity: 0.7;">${escapeHtml(suggestion.formattedString.split(' um ')[0] || '')}</div>
                            </div>
        ...
        selectSlot(suggestion) {
            ...
            if (displayElement) {
                displayElement.innerHTML = `<strong>${escapeHtml(suggestion.formattedString)}</strong>`;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (confirmationDetails) {
                        let html = `
                            <p><strong>Termin:</strong> ${escapeHtml(data.appointmentDetails?.formattedDate || state.selectedCallbackData.formattedString)}</p>
                            <p><strong>Uhrzeit:</strong> ${escapeHtml(data.appointmentDetails?.formattedTime || '')}</p>
                            <p><strong>Name:</strong> ${escapeHtml(name)}</p>
                            <p><strong>Telefon:</strong> ${escapeHtml(phone)}</p>
                            ${topic ? `<p><strong>Anliegen:</strong> ${escapeHtml(topic)}</p>` : ''}
                        `;
                        ...
                        html += `</div>`;
                        confirmationDetails.innerHTML = sanitizeHtml(html);
                    }
