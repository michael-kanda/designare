    ...
    function updateRatioBar(container, percentages, total) {
        ...
        if (labels) {
            ...
            if (total > 0) {
                ...
                left.textContent = '👍 ' + String(percentages.positive) + '% hilfreich';
                var right = document.createElement('span');
                right.textContent = String(total) + ' Bewertung' + (total !== 1 ? 'en' : '');
                labels.appendChild(left);
                labels.appendChild(right);
            } else {
                var emptyLeft = document.createElement('span');
                emptyLeft.textContent = 'Noch keine Bewertungen';
                var emptyRight = document.createElement('span');
                emptyRight.textContent = 'Sei der Erste!';
                labels.appendChild(emptyLeft);
                labels.appendChild(emptyRight);
            }
        }
    }
