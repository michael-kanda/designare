...
${additionalContext}
═══════════════════════════════════════${turnCount > 5 ? '\nERINNERUNG: Beantworte Fragen über Michael/designare.at NUR auf Basis des WEBSEITEN-KONTEXT oben. Nichts erfinden!' : ''}` : 'WEBSEITEN-KONTEXT: Keiner verfügbar. Bei Fragen über Michael/designare.at ehrlich sagen: "Da bin ich mir nicht sicher – frag am besten Michael direkt!"'}
${availableLinks.length > 0 ? `\nVERFÜGBARE LINKS:\n${availableLinks.map(l => `- ${l.url} → "${l.title}"`).join('\n')}` : '\nVERFÜGBARE LINKS: KEINE. Generiere KEINE Link-Chips in dieser Runde.'}`;
}
