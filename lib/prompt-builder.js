...
${weekendContext ? `${weekendContext}\n` : ''}${isFirstMessage && timeContext ? `${timeContext}\n` : ''}${holidayContext ? `${holidayContext}\n` : ''}${weatherContext ? `WETTER: ${weatherContext}\n` : ''}${newsContext ? `TECH-NEWS HEUTE: ${newsContext}\nWenn der Nutzer mehr Details zu einer News will, verweise locker auf die Originalquellen (z.B. "Schau mal bei heise oder t3n vorbei, da findest du die ganze Story"). Du bist kein Nachrichtenportal – gib die Richtung, nicht den Artikel.\n` : ''}
${memoryContext}
