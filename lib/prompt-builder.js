...
// Baut den Prompt aus Zeit, Memory, RAG-Kontext und Konfiguration zusammen
...
export function buildSystemPrompt({
  ...
  if (isReturningUser && knownName) {
    ...
    memoryContext = `WIEDERKEHRENDER BESUCHER: ${knownName} (Besuch ${visitCount}, zuletzt vor ${timeSince}). Begrüße mit "${timeGreeting}, ${knownName}!" – NICHT nach dem Namen fragen.`;
    if (previousTopics.length > 0) {
      memoryContext += ` Frühere Themen: ${previousTopics.slice(-5).join(', ')}`;
    }
  } else if (isReturningUser) {
    memoryContext = `WIEDERKEHRENDER BESUCHER (Name unbekannt, Besuch ${visitCount}). Begrüße mit "${timeGreeting}!".`;
  } else {
