 ...
 * NEU: Begrenzt Nachrichtenlänge + Gesamtlänge als Sicherheitsnetz.
 */
export function buildChatContents(history, userMessage) {
  const contents = [];

  // Sicherheitsnetz: Einzelne Nachrichten und Gesamt-Kontext begrenzen
  // Verhindert dass lange E-Mail-Drafts oder Antworten das Context-Window sprengen
  ...
  if (history && Array.isArray(history) && history.length > 0) {
    // Rückwärts iterieren: neueste Nachrichten zuerst einsammeln,
    // älteste werden bei Limit-Überschreitung abgeschnitten
    const cleaned = [];
